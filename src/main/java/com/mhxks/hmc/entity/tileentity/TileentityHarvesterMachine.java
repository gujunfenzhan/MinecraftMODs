...
implements ITickable{
    ...
    public void receCropToChest(BlockPos cropPos,int grow){
        ...
        AxisAlignedBB AABB = new AxisAlignedBB(cropPos).grow(0,grow,0);

        List<EntityItem> itemList = world.getEntitiesWithinAABB(EntityItem.class, AABB);
        for (EntityItem entityItem : itemList) {
            IInventory iInventory = getNearChest();
            if (iInventory == null) {
                world.spawnEntity(new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), entityItem.getItem()));
            } else {
                //把物品放进箱子里
                ItemStack stack = putStackInInventoryAllSlots(iInventory, entityItem.getItem());
                if (!stack.isEmpty()) {
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        return super.getRenderBoundingBox().grow(8,0,8);
    }
