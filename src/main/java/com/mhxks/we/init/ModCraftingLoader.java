...
public class ModCraftingLoader {
    ...
    public void addWorkbenchEnchanting(Enchantment enchantment, ItemStack itemStack){
        ...
        for (int i =1; i <=maxlevel; i++) {//循环添加这个合成表
            ...
            items[i] = Ingredient.fromStacks(itemStack);
            items[i+1] = Ingredient.fromItem(Items.BOOK);
            ...
            GameRegistry.addShapelessRecipe(new ResourceLocation(WorkbenchEnchantingMain.MODID+":"+getId()),group,fumoshu,items);
        }
